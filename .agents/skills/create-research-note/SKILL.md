...
9. For durable PDF excerpts, use `type: evidence` with `source.pdf` and `source.page`; keep the exact excerpt in a blockquote and interpretation outside the quote.
10. Clearly separate evidence, interpretation, limitations, and next steps when those distinctions matter.
11. Run `npm run doctor`.
12. If validation fails because of the new note, repair the note rather than weakening the doctor.
