import { spawn } from "node:child_process";
import crypto from "node:crypto";
import fs from "node:fs/promises";
import os from "node:os";
import path from "node:path";

const PROPOSAL_MAX_AGE_MS = 7 * 24 * 60 * 60 * 1000;
const PROPOSAL_MAX_COUNT = 100;

function run(command, args, { cwd, input, timeoutMs = 120000 } = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn(command, args, {
      cwd,
      env: process.env,
      stdio: ["pipe", "pipe", "pipe"],
    });
    const stdout = [];
    const stderr = [];
    const timer = setTimeout(() => child.kill("SIGTERM"), timeoutMs);

    child.stdout.on("data", (chunk) => stdout.push(chunk));
    child.stderr.on("data", (chunk) => stderr.push(chunk));
    child.on("error", (error) => {
      clearTimeout(timer);
      reject(error);
    });
    child.on("close", (code, signal) => {
      clearTimeout(timer);
      resolve({
        code: code ?? (signal ? 1 : 0),
        stdout: Buffer.concat(stdout).toString("utf8"),
        stderr: Buffer.concat(stderr).toString("utf8"),
      });
    });

    if (input) child.stdin.write(input);
    child.stdin.end();
  });
}

export async function runGit(cwd, args, options = {}) {
  return run("git", args, { cwd, ...options });
}

export function parseStatusPaths(output) {
  const paths = [];
  for (const record of output.split("\0").filter(Boolean)) {
    if (/^[ MADRCU?!]{2} /.test(record)) paths.push(record.slice(3));
    else paths.push(record);
  }
  return [...new Set(paths.filter(Boolean))];
}

function normalizeRepoPath(value) {
  const normalized = path.posix.normalize(String(value ?? "").replace(/\\/g, "/").replace(/^\.\//, ""));
  if (!normalized || normalized === "." || normalized === ".." || normalized.startsWith("../") || path.posix.isAbsolute(normalized)) {
    throw new Error("Research path must stay inside the repository.");
  }
  return normalized;
}

function safeFilePath(value) {
  const normalized = normalizeRepoPath(value);
  if (normalized.endsWith("/")) throw new Error("Proposal file path must identify a file.");
  return normalized;
}

export function allResearchPaths(paths, researchPath = "progress") {
  const root = normalizeRepoPath(researchPath);
  return paths.length > 0 && paths.every((file) => {
    const normalized = safeFilePath(file);
    if (!(normalized === root || normalized.startsWith(root + "/"))) return false;
    return !/(^|\/)AGENTS\.md$/i.test(normalized);
  });
}

export async function gitStatusPaths(root) {
  const result = await runGit(root, ["status", "--porcelain=v1", "-z", "--untracked-files=all"]);
  if (result.code !== 0) throw new Error(result.stderr || "git status failed");
  return parseStatusPaths(result.stdout);
}

export async function withDetachedWorktree(root, task) {
  const parent = path.join(os.tmpdir(), "research-observer-codex");
  await fs.mkdir(parent, { recursive: true });
  const worktree = path.join(parent, crypto.randomUUID());

  const add = await runGit(root, ["worktree", "add", "--detach", worktree, "HEAD"], { timeoutMs: 60000 });
  if (add.code !== 0) throw new Error(add.stderr || "Could not create Codex worktree.");

  try {
    return await task(worktree);
  } finally {
    await runGit(root, ["worktree", "remove", "--force", worktree], { timeoutMs: 60000 }).catch(() => null);
    await fs.rm(worktree, { recursive: true, force: true }).catch(() => null);
  }
}

function nulPaths(output) {
  return output.split("\0").filter(Boolean);
}

export async function collectResearchDiff(worktree, researchPath = "progress") {
  const tracked = await runGit(worktree, ["diff", "--name-only", "-z", "--"]);
  if (tracked.code !== 0) throw new Error(tracked.stderr || "Could not inspect Codex working-tree changes.");
  const untracked = await runGit(worktree, ["ls-files", "--others", "--exclude-standard", "-z"]);
  if (untracked.code !== 0) throw new Error(untracked.stderr || "Could not inspect new Codex files.");

  const untrackedFiles = nulPaths(untracked.stdout);
  const files = [...new Set([...nulPaths(tracked.stdout), ...untrackedFiles])];
  if (!files.length) return { files: [], patch: "", allowed: true, reviewable: true, patchBytes: 0, binary: false };

  const allowed = allResearchPaths(files, researchPath);
  if (allowed && untrackedFiles.length) {
    const intent = await runGit(worktree, ["add", "-N", "--", ...untrackedFiles]);
    if (intent.code !== 0) throw new Error(intent.stderr || "Could not prepare new research files for diff.");
  }

  const diff = await runGit(worktree, ["diff", "--binary", "--no-ext-diff", "--"]);
  if (diff.code !== 0) throw new Error(diff.stderr || "Could not collect Codex proposal diff.");
  const patchBytes = Buffer.byteLength(diff.stdout, "utf8");
  const binary = diff.stdout.includes("GIT binary patch");
  const reviewable = patchBytes <= 120000 && !binary;
  return { files, patch: diff.stdout, allowed, reviewable, patchBytes, binary };
}

export async function captureTreeFileStates(worktree, treeSha, files) {
  const states = Object.fromEntries(files.map((file) => [safeFilePath(file), null]));
  const normalizedFiles = Object.keys(states);
  if (!normalizedFiles.length) return states;
  if (!/^(?:[0-9a-f]{40}|[0-9a-f]{64})$/i.test(String(treeSha ?? ""))) throw new Error("Invalid review baseline tree.");

  const result = await runGit(worktree, ["ls-tree", "-z", String(treeSha), "--", ...normalizedFiles]);
  if (result.code !== 0) throw new Error(result.stderr || "Could not capture review baseline file states.");
  for (const record of result.stdout.split("\0").filter(Boolean)) {
    const tab = record.indexOf("\t");
    if (tab < 0) continue;
    const metadata = record.slice(0, tab).split(/\s+/);
    const file = record.slice(tab + 1);
    if (metadata[1] === "blob" && metadata[2] && Object.hasOwn(states, file)) states[file] = metadata[2];
  }
  return states;
}

async function liveFileState(root, file) {
  const normalized = safeFilePath(file);
  const absolute = path.join(root, ...normalized.split("/"));
  let stat;
  try {
    stat = await fs.lstat(absolute);
  } catch (error) {
    if (error?.code === "ENOENT") return null;
    throw error;
  }
  if (!stat.isFile()) return `unsafe:${stat.isSymbolicLink() ? "symlink" : "non-file"}`;
  const hashed = await runGit(root, ["hash-object", "--", normalized]);
  if (hashed.code !== 0) throw new Error(hashed.stderr || `Could not hash ${normalized}.`);
  return hashed.stdout.trim();
}

export async function changedFileStates(root, baseline = {}) {
  const changed = [];
  for (const [file, expected] of Object.entries(baseline ?? {})) {
    const normalized = safeFilePath(file);
    const current = await liveFileState(root, normalized);
    if (current !== expected) changed.push(normalized);
  }
  return changed;
}

export async function runResearchDoctor(root, cwd = root) {
  const script = path.join(root, "scripts", "research-doctor.mjs");
  return run(process.execPath, [script], { cwd, timeoutMs: 120000 });
}

function draftDirectory(root) {
  return path.join(root, ".research-observer", "codex-drafts");
}

async function pruneDraftDirectory(directory) {
  let files;
  try {
    files = await fs.readdir(directory);
  } catch (error) {
    if (error?.code === "ENOENT") return;
    throw error;
  }

  const metadataFiles = files.filter((file) => /^[0-9a-f-]{36}\.json$/i.test(file));
  const records = (await Promise.all(metadataFiles.map(async (file) => {
    try {
      const stat = await fs.stat(path.join(directory, file));
      return { id: file.slice(0, -5), mtimeMs: stat.mtimeMs };
    } catch {
      return null;
    }
  }))).filter(Boolean).sort((a, b) => b.mtimeMs - a.mtimeMs);

  const now = Date.now();
  const staleIds = records
    .filter((record, index) => index >= PROPOSAL_MAX_COUNT || now - record.mtimeMs > PROPOSAL_MAX_AGE_MS)
    .map((record) => record.id);

  await Promise.all(staleIds.flatMap((id) => [
    fs.rm(path.join(directory, id + ".json"), { force: true }),
    fs.rm(path.join(directory, id + ".patch"), { force: true }),
  ])).catch(() => null);

  const knownIds = new Set(records.map((record) => record.id));
  const orphanPatches = files
    .filter((file) => /^[0-9a-f-]{36}\.patch$/i.test(file))
    .map((file) => file.slice(0, -6))
    .filter((id) => !knownIds.has(id));
  await Promise.all(orphanPatches.map((id) => fs.rm(path.join(directory, id + ".patch"), { force: true }))).catch(() => null);
}

export async function storeProposal(root, proposal) {
  const id = crypto.randomUUID();
  const directory = draftDirectory(root);
  await fs.mkdir(directory, { recursive: true });
  await pruneDraftDirectory(directory).catch(() => null);
  const patchPath = path.join(directory, id + ".patch");
  const metaPath = path.join(directory, id + ".json");

  const metadata = {
    id,
    kind: proposal.kind || "codex",
    createdAt: new Date().toISOString(),
    files: proposal.files,
    valid: proposal.valid,
    reviewable: proposal.reviewable,
    doctor: proposal.doctor,
    summary: proposal.summary,
    ...(proposal.slug ? { slug: proposal.slug } : {}),
    ...(proposal.filename ? { filename: proposal.filename } : {}),
    ...(proposal.baseSha256 ? { baseSha256: proposal.baseSha256 } : {}),
    ...(proposal.baseFiles ? { baseFiles: proposal.baseFiles } : {}),
    ...(proposal.researchPath ? { researchPath: proposal.researchPath } : {}),
    ...(proposal.workspaceSignature ? { workspaceSignature: proposal.workspaceSignature } : {}),
    patchSha256: crypto.createHash("sha256").update(proposal.patch, "utf8").digest("hex"),
  };

  await Promise.all([
    fs.writeFile(patchPath, proposal.patch, "utf8"),
    fs.writeFile(metaPath, JSON.stringify(metadata, null, 2) + "\n", "utf8"),
  ]);
  return metadata;
}

export async function loadProposal(root, id) {
  if (!/^[0-9a-f-]{36}$/i.test(id)) throw new Error("Invalid proposal ID.");
  const directory = draftDirectory(root);
  const [metadataRaw, patch] = await Promise.all([
    fs.readFile(path.join(directory, id + ".json"), "utf8"),
    fs.readFile(path.join(directory, id + ".patch"), "utf8"),
  ]);
  return { metadata: JSON.parse(metadataRaw), patch };
}

export async function deleteProposal(root, id) {
  const directory = draftDirectory(root);
  await Promise.all([
    fs.rm(path.join(directory, id + ".json"), { force: true }),
    fs.rm(path.join(directory, id + ".patch"), { force: true }),
  ]);
}
