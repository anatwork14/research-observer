import { consensusIntegrationStatus, resolveConsensusApiKey } from "../settings/integrations.mjs";
...
export function normalizeConsensusPaper(raw = {}) {
  ...
  const url = secureUrl(first(raw.url, raw.consensus_url, metadata.url));
  const doi = normalizeDoi(first(raw.doi, metadata.doi));
...
export function normalizeConsensusSearch(payload = {}) {
  const nestedData = !Array.isArray(payload) && payload.data && typeof payload.data === "object"
    ? payload.data
    : {};
  const candidate = Array.isArray(payload)
    ? payload
    : first(payload.papers, payload.results, nestedData.papers, nestedData.results, payload.data, []);
  ...
  return {
    ...
    papers: rows
      .map(normalizeConsensusPaper)
      .filter((paper) => paper.title && (paper.url || paper.doi || paper.id)),
  };
}
...
export async function searchConsensus(input = {}, options = {}) {
  const resolved = await resolveConsensusApiKey({
    rootDir: options.rootDir ?? process.cwd(),
    explicit: options.apiKey,
  });
  const apiKey = cleanString(resolved.apiKey);
  if (!apiKey) {
    const error = new Error("Consensus is not configured. Add an API key in Settings or set CONSENSUS_API_KEY on the server.");
    error.code = "CONSENSUS_NOT_CONFIGURED";
    throw error;
  }

  const configuredBaseUrl = cleanString(options.baseUrl ?? process.env.CONSENSUS_API_BASE_URL);
...
export async function consensusConfigured(options = {}) {
  return (await consensusIntegrationStatus(options)).configured;
}

export async function consensusConfiguration(options = {}) {
  const state = await consensusIntegrationStatus(options);
  return {
