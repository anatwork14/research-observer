...
function relationshipFor(workspace, relationship) {
  ...
  const canonical = workspace.entries.find((entry) => entry.slug === target || entry.aliases.includes(target));
  const filenameMatches = canonical ? [] : workspace.entries.filter((entry) => entry.fileSlug === target);
  if (!canonical && filenameMatches.length > 1) {
    throw new Error("Relationship target is ambiguous across research projects. Use the target note's stable id.");
  }
  const targetEntry = canonical || filenameMatches[0];
