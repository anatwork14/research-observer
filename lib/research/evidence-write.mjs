...
export async function createConsensusEvidenceNote({
  ...
  const doi = cleanString(paper.doi, 500);
  ...
  const declaredResearch = workspace.config.researchProjects.some((project) => project.id === requested) ? requested : undefined;
  ...
  const filename = `${String(nextOrder).padStart(2, "0")}_evidence_consensus_${base}_${suffix}.md`;
