...
async function jsonRows(file, jsonl) {
  ...
  if (!jsonl) {
    ...
    let pending = "";
    const acceptLine = (line) => { if (!line.trim()) return; const row = JSON.parse(line); if (!safeDepth(row)) throw new Error("JSON nesting limit exceeded."); rows.push(row); if (rows.length > IMPORT_LIMITS.rows) throw new Error("Import row limit exceeded."); };
    ...
    pending += decoder.decode(); acceptLine(pending);
  }
