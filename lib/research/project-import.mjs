...
function incomingProjectId(prepared, directory) {
  const manifestFile = prepared.find((file) => file.relativePath === PROJECT_MANIFEST);
  if (!manifestFile) return projectIdFromFolder(directory);
  try {
    const parsed = JSON.parse(manifestFile.data.toString("utf8"));
    if (parsed && typeof parsed === "object" && typeof parsed.id === "string" && PROJECT_ID.test(parsed.id)) {
      return parsed.id;
    }
  } catch {
    // Canonical compiler validation will report the malformed manifest after staging.
  }
  return projectIdFromFolder(directory);
}
...
export async function importResearchProject({ rootDir = process.cwd(), projectName, files = [] } = {}) {
  ...
  const incomingId = incomingProjectId(prepared, directory);
  const existingIdentity = workspace.projects.find((project) => project.id === incomingId && project.notes > 0);
  if (existingIdentity) {
    throw importError(
      `Project id "${incomingId}" is already used by "${existingIdentity.label}". Rename the folder or provide a different id in ${PROJECT_MANIFEST}.`,
  ...
  try {
    ...
    const project = compiled.projects.find((item) => item.directory === directory) ?? compiled.projects.find((item) => item.id === incomingId);
