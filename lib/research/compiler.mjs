...
function sourceValue(value, file, diagnostics) {
  ...
  if (!pdf) diagnostics.push(diag("error","source-invalid","Evidence source requires a pdf path.",file));
...
export async function compileResearchWorkspace({rootDir=process.cwd(),fresh=false}={}) {
  ...
  for (const f of noteFiles) {
    ...
    entries.push({
      ...
      type,status,date:dateValue(data.date,filename,diagnostics),tags,authors,year:yearValue(data.year,filename,diagnostics),doi,pdf,source,content,text:strip(content),words,
  ...
  for(const e of entries) {
    for (const relation of e._relationshipsRaw) {
      const rawTarget=relation.target.replace(/\.md$/i,"");
      const found=filesBySlug.get(rawTarget)||routes.get(rawTarget);
      if (!found) {
  ...
  const graph={
    nodes:entries.map((e)=>({slug:e.slug,title:e.title,type:e.type,status:e.status,order:e.order})),
    edges:[
  ...
  const health={
    ...
    evidenceMissingSource:entries.filter((e)=>e.type==="evidence"&&!e.source?.pdf).map((e)=>e.slug),
  ...
  const workspace={schemaVersion:2,rootDir:root,progressRoot,config,signature,entries,assets,graph,health,diagnostics,stats:{
...
export async function writeResearchArtifacts(options={}) {
  ...
  const search=w.entries.map((e)=>({slug:e.slug,order:e.order,title:e.title,summary:e.summary,type:e.type,status:e.status,date:e.date,tags:e.tags,authors:e.authors,year:e.year,doi:e.doi,pdf:e.pdf,source:e.source,relationships:e.relationships,headings:e.headings,text:e.text}));
  await Promise.all([
    fs.writeFile(path.join(dir,"manifest.json"),JSON.stringify({schemaVersion:2,signature:w.signature,stats:w.stats,entries,assets:w.assets,health:w.health,diagnostics:w.diagnostics},null,2)+"\n"),
    fs.writeFile(path.join(dir,"search.json"),JSON.stringify({schemaVersion:2,signature:w.signature,entries:search})+"\n"),
