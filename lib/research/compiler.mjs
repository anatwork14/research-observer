...
export async function compileResearchWorkspace({rootDir=process.cwd(),fresh=false}={}) {
  ...
  for (const f of noteFiles) {
    ...
    let research=stringValue(data.research,"research",filename,diagnostics)||"default";
    if (!ID.test(research)) {
    ...
    if (!config.researchProjects.some((project)=>project.id===research)) {
      diagnostics.push(diag(vocabularySeverity,"research-project-unknown",'Research project "'+research+'" is not declared in researchProjects.',filename));
    }
