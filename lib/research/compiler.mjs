...
export async function compileResearchWorkspace({rootDir=process.cwd(),fresh=false}={}) {
  ...
  for (const f of noteFiles) {
    ...
    entries.push({
      ...
      type,status,date:dateValue(data.date,filename,diagnostics),tags,authors,year:yearValue(data.year,filename,diagnostics),doi,pdf,content,text:strip(content),words,
...
export async function writeResearchArtifacts(options={}) {
  ...
  const search=w.entries.map((e)=>({slug:e.slug,order:e.order,title:e.title,summary:e.summary,type:e.type,status:e.status,date:e.date,tags:e.tags,authors:e.authors,year:e.year,doi:e.doi,pdf:e.pdf,headings:e.headings,text:e.text}));
  await Promise.all([
