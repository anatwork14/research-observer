...
export async function applyDirectEdit({ rootDir = process.cwd(), id } = {}) {
  ...
  const updatedEntry = compiled.entries.find((item) => item.filename === metadata.filename);
  if (!updatedEntry) {
    await runGit(root, ["apply", "-R", "--whitespace=nowarn", "-"], { input: patch });
    await writeResearchArtifacts({ rootDir: root, fresh: true }).catch(() => null);
    throw new Error("The edited note no longer resolves after compilation; the change was rolled back.");
  }
