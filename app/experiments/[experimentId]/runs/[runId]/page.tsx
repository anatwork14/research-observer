...
import styles from "@/components/ExperimentWorkbench.module.css";
...
export default async function RunDetailPage({ params }: { params: Promise<{ experimentId: string; runId: string }> }) {
  ...
  return <div className="site-shell"><WorkspaceHeader entries={workspace.entries.map(({ slug, order, title, status }) => ({ slug, order, title, status }))} active="projects" /><main className={`collection-shell ${styles.runDetailPage}`}>
    <header className="collection-heading"><div><p className="eyebrow"><Link href={`/projects/${encodeURIComponent(experiment.research)}/experiments`}>{project?.label ?? experiment.research} · Experiments</Link> / <Link href={`/experiments/${encodeURIComponent(experiment.id ?? experiment.slug)}`}>{experiment.title}</Link></p><h1>{run.label}</h1><p className={styles.runMeta}><code>{run.id}</code><span>{run.status}</span><time>{run.timestamps?.createdAt ?? "timestamp unavailable"}</time></p></div><Link href={`/projects/${encodeURIComponent(experiment.research)}/experiments?view=compare`}>Compare this run</Link></header>
    <section className="panel"><h2>Summary metrics and provenance</h2><div className={styles.tableScroll}><table><thead><tr><th>Metric</th><th>Value</th><th>Definition</th><th>Provenance</th></tr></thead><tbody>{Object.entries(run.metrics ?? {}).map(([id, item]) => { const definition = definitions.get(id); const source = item.source ?? {}; return <tr key={id}><th><span className={styles.metricIdentity}><strong>{definition?.label ?? id}</strong><code>{id}</code><small>{definition ? `${definition.direction} · ${definition.unit}` : "Metric missing from plan"}</small></span></th><td><span className={styles.metricValue}>{item.value}</span> {definition?.unit ?? ""}</td><td>{definition ? `${definition.role} · ${definition.aggregation}` : "Metric missing from plan"}</td><td>{source.kind === "manual" ? `Manual: ${String(source.note ?? "")}` : `${String(source.file ?? "source unavailable")} · ${String(source.column ?? "column unavailable")} · ${String(source.aggregation ?? "aggregation unavailable")}`}</td></tr>; })}</tbody></table></div></section>
    <section className="panel"><h2>Parameters</h2><dl>{Object.entries(run.parameters ?? {}).map(([name, value]) => <div className={styles.parameterRow} key={name}><dt>{name}</dt><dd>{JSON.stringify(value)}</dd></div>)}</dl>{!Object.keys(run.parameters ?? {}).length && <p>No parameters recorded.</p>}</section>
    ...
    <section className="panel"><h2>Metric values</h2><div className={styles.tableScroll}><table><thead><tr><th>Metric</th><th>Value</th><th>Unit</th><th>Source</th></tr></thead><tbody>{Object.entries(run.metrics ?? {}).map(([id, item]) => <tr key={id}><th><span className={styles.metricIdentity}><strong>{definitions.get(id)?.label ?? id}</strong><code>{id}</code></span></th><td><span className={styles.metricValue}>{item.value}</span></td><td>{definitions.get(id)?.unit ?? "—"}</td><td>{String(item.source?.file ?? item.source?.kind ?? "—")}</td></tr>)}</tbody></table></div></section>
