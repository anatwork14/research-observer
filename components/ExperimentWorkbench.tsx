...
export function ExperimentWorkbench(props: Props) {
  ...
  return <section className={styles.workbench}>
    ...
    {activeView === "overview" && <>
      <div className={styles.cards}><article><small>Experiments</small><strong>{props.experiments.length}</strong></article><article><small>Runs</small><strong>{activeRuns.length}</strong></article><article><small>Primary metrics</small><strong>{primary.length}</strong></article><article><small>Guardrail checks</small><strong className={violations.length ? styles.bad : ""}>{violations.length ? `${violations.length} flagged` : "Within thresholds"}</strong></article></div>
  ...
  return <section className="panel"><div className={styles.sectionHead}><div><h2>Run data</h2><p>Add a manual measurement or preview an import before confirming its durable write.</p></div><div className={styles.switch}><button className={mode === "manual" ? styles.selected : ""} onClick={() => setMode("manual")}>Manual</button><button className={mode === "import" ? styles.selected : ""} onClick={() => setMode("import")}>Import file</button></div></div>
    ...
    {mode === "import" && <div className={styles.formStack}><label className={styles.formField}>Original data file<input type="file" accept=".csv,.tsv,.json,.jsonl" onChange={(event) => { setFile(event.target.files?.[0] ?? null); setPreview(null); }} /></label><button disabled={!file || busy || !selectedExperiment} onClick={() => void previewImport()}>{busy ? "Reading…" : "Preview rows"}</button>
      {preview && <>
        ...
        <button disabled={busy || !rowMode} onClick={() => void confirmImport()}>{busy ? "Saving…" : "Confirm and save run data"}</button>
      </>}
    </div>}
  </section>;
}
